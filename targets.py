...
class TargetMixture(object):
    ...
    def Gaussian(self, length):
        self.Gaussians = np.random.multivariate_normal(mean = np.zeros(self.dimension), cov = np.identity(self.dimension), size = length)
        self.index = 0
    
    def Gaussian_next(self, num = 1):
        if self.index + num >= len(self.Gaussians):
            self.Gaussian(len(self.Gaussians))
            self.index = num
            return self.Gaussians[0:num,:] # num < len(self.Gaussians)
        else:
            self.index = self.index + num
            return self.Gaussians[self.index-num:self.index,:]
    ...
    def solve1(self, input, eta):
        ...
        x0 = input
        # res = minimize(targetFunction, x0, method='nelder-mead', options={'fatol': 1e-8, 'disp': False, 'adaptive' : True})
        # res = minimize(targetFunction, x0, method='Powell', options={'disp': False})
        res = minimize(targetFunction, x0, method='L-BFGS-B', options={'gtol': 1e-4, 'disp': False, 'maxiter': 10})
...
class SemiGaussianTarget(object):
    ...
    def firstOrder(self, x):
        ...
        temp_vector = x-self.mean
        norm = np.sqrt(np.dot(temp_vector,temp_vector))
        return 0.5*(self.alpha+1) * (x-self.mean) * norm**(self.alpha-1)
...
class NFarget(object):
    ...
    def Gaussian_next(self, num = 1):
        if self.index + num >= len(self.Gaussians):
            self.Gaussian(10000)
            self.index = num
            return self.Gaussians[0:num,:] # num < 10000
        else:
            self.index = self.index + num
            return self.Gaussians[self.index-num:self.index,:]