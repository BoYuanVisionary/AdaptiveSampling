# Define a function that generates samples from a one-dimentional Mixture Cauchy with approximate RGO 
# The target is 1/(1+(x-1)^2)+1/(1+(x+1)^2)
import numpy as np
...
def estimate_step_size(step_size, tolerance, y):
    ...
    while True:
        ...
        left = 0
        right = 1
        while np.mean(np.exp(np.square(Y)/right))-2>0:
            left = right
            right = 2*right
        # Initialize the middle point
        mid = (left+right)/2
        # Initialize the value of the function
        f_mid = np.mean(np.exp(np.square(Y)/mid))-2
...
def proximal_sampler(initial_step_size, num_samples, num_iter):
    ...
    times = np.zeros([num_samples,num_iter])
    for i in range(1,num_iter):
        for j in range(num_samples):
            # Compute the new stationary point
            step_size, x_y, times[j,i]  = estimate_step_size(step_sizes[j,i-1], 1e-3, Ysamples[j])
    ...
    #         print(i,j)
    return samples, step_sizes, times

# Define the potential function and its gradient
class potential(object):
    def zero_order(self, x):
        f_x = np.log(1+(x+1)**2)-np.log((4+2*x**2)/(1+(x-1)**2))
        return f_x
    def first_order(self, x):
        df_x = 2*(x+1)/(1+(1+x)**2)-(4-2*x**2)/ ((2+x**2)*(1+(x-1)**2))
        return df_x
    ...
    def solve(self, y, eta):
        ...
        left = min(0,y)
        right = max(0,y)
        while (self.first_order(left)+(1/eta)*(left-y)) * (self.first_order(right)+(1/eta)*(right-y)) > 0:
            interval = right - left
            left = left - interval
        ...
        return mid
num_iter = 100
Xsamples, step_sizes, times = proximal_sampler(100, num_samples = 1000, num_iter = num_iter)
constant = 2 * np.pi
...
for i in range(num_iter):
    ...
    def l1_distance(x):
        f_x =  (1/(1+(x-1)**2)+1/(1+(x+1)**2)) / constant
...
plt.show()
plt.plot(np.mean(times,axis=0))
plt.show()
# plt.hist(Xsamples[:,num_iter-1], bins=20, edgecolor='black')
# plt.show()
