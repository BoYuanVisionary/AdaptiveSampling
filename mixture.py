...
def diagnosis(target, numIter, initialStep, numSamples, fixed, direction, distance):
    ...
    number_samples_hist = int(3e4)
    min_number_samples_hist = int(1e4)
    number_hist = 100
    ...
    if numIter > number_samples_hist:
        ...
        plt.savefig( "TV_along_direction_1.png")
        plt.show()
        plt.close()
    ...
    if numIter > number_samples_hist:
        ...
        for i in range(number_hist):
            distances_random[i] = TV_estimation(projected_sample_random[-index_hist[i]:], histY, bin_edges_Y, bin_num)
        plt.plot(0.5*distances_random)
        plt.title('TV along distance')
        plt.savefig( "TV_along_distance_2.png")
        plt.show()
        plt.close()
    ...
    if numIter > number_samples_hist:
        ...
        plt.savefig( "TV_along_orthogonal.png")
        plt.show()
        plt.close()
    ...
    plt.savefig( "mean_step_size.png")
    plt.show()
    plt.close()
...
@measure_time
def diagnosis_MALA(target, step_size, numIter, numSamples, direction, distance):
    numIter = numIter
    step_size = step_size
    numSamples = numSamples

    f = target
    Xsamples = MALA(f,step_size, numIter , numSamples)

    number_samples_hist = int(3e5)
    min_number_samples_hist = int(1e5)
    number_hist = 100
    # Not the precise bin_size but it doesn't matter
    bin_size = int((number_samples_hist-min_number_samples_hist) / 100 )
    index_hist = np.linspace(min_number_samples_hist, number_samples_hist, number_hist)
    ...
    if numIter > number_samples_hist:
        ...
        for i in range(number_hist):
            distances_random[i] = TV_estimation(projected_sample_random[-index_hist[i]:], histY, bin_edges_Y, bin_num)
        plt.plot(0.5*distances_random)
        plt.title('TV along direction (MALA)')
        plt.show()
        plt.close()
    ...
    if numIter > number_samples_hist:
        ...
        for i in range(number_hist):
            distances_random[i] = TV_estimation(projected_sample_random[-index_hist[i]:], histY, bin_edges_Y, bin_num)
        plt.plot(0.5*distances_random)
        plt.title('TV along distance (MALA)')
        plt.show()
        plt.close()    
    # 1.3 Projection along 'orthogonal'
    random_vector = np.random.rand(f.dimension)
    ...
    if numIter > number_samples_hist:
        ...
        for i in range(number_hist):
            distances_random[i] = TV_estimation(projected_sample_random[-index_hist[i]:], histY, bin_edges_Y, bin_num)
        plt.plot(0.5*distances_random)
        plt.title('TV along orthogonal (MALA)')
        plt.show()
        plt.close()
    ...
    # 4. Histogram at the last iteration
    if numSamples > 100 :
        plt.hist(np.matmul(Xsamples[:,numIter-1,:],direction), bins=100, edgecolor='black', density=True)
        plt.show()
        plt.close()
...
def main(seed,dimension,large_condition):
    ...
    specialMean = np.random.multivariate_normal(mean = np.zeros(dimension), cov = np.identity(dimension), size = 2)
    specialInverse = generateInvertibleMatrixConditional(dimension,large_condition)
    ...
    t1 = targets.SemiGaussianTarget(dimension = dimension, mean = specialMean[0,:], inverse = specialInverse, alpha = np.random.uniform(0,1) , prob = prob1)
    t2 = targets.SemiGaussianTarget(dimension = dimension, mean = specialMean[0,:]- distance, inverse = specialInverse, alpha = np.random.uniform(0,1) , prob = prob2)
    t3 = targets.SemiGaussianTarget(dimension = dimension, mean = specialMean[0,:]- 2*distance, inverse = specialInverse, alpha = 1 , prob = prob3)
    Target = targets.TargetMixture(t1,t2,t3)

    # # Define a Guassian distribution
    # t = targets.SemiGaussianTarget(dimension = dimension, mean = specialMean[0,:], inverse = specialInverse, alpha = 1, prob = 1)
    # Target = targets.TargetMixture(t)
    ...
    # Comment out this line if MALA is not required
    print(f"the last step size:{np.mean(stepSizes[:,-1])}")
    # Xsamples_MALA  = diagnosis_MALA(target =Target, step_size = np.mean(stepSizes[:,-1]), numIter = numIter, numSamples = numSamples, direction=direction, distance=distance)
    # Fixed one
...
def generateInvertibleMatrixConditional(dimension,large_condition):
    ...
    if large_condition == True:
        temp = np.matmul(random_orthogonal_matrix, np.diag(np.sqrt(np.linspace(0.5,10,num=dimension))))
    else:
        temp = np.matmul(random_orthogonal_matrix, np.sqrt(1)*np.identity(dimension))
...
cProfile.run("samples, direction, _ = main(seed,dimension,large_condition)", sort="cumulative", filename="profile_results")
stats = pstats.Stats("profile_results")
# Print the top 30 results by cumulative time
stats.sort_stats("cumulative").print_stats(30)
# main(seed,dimension,large_condition)
